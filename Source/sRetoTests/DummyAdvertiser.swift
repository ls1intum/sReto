...
class DummyAdvertiser: NSObject, Advertiser  {
    ...
    func startAdvertising(identifier : UUID) {
        ...
        dispatch_async(dispatch_get_main_queue(), { () -> Void in
            if let adv = self.advertiserDelegate { adv.didStartAdvertising(self) }
        })
    }
    func stopAdvertising() {
        ...
        dispatch_async(dispatch_get_main_queue(), { () -> Void in
            if let adv = self.advertiserDelegate { adv.didStopAdvertising(self) }
        })
    }
    
    func onConnection(connection: DummyInConnection) {
        dispatch_async(dispatch_get_main_queue(), { () -> Void in
            if let adv = self.advertiserDelegate { adv.handleConnection(self, connection: connection) }
        })
    }
}